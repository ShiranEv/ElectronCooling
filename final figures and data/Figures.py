...
def plot_v0_L_map(csv_path, initial_width, vg_fixed, E_rel, Loss):
    ...
    width_pivot = df.pivot(index="L_int_m", columns="v_0_m_per_s", values="width").reindex(
        index=L_vals, columns=v0_vals
    )
    ...
    print(
        f"Minimum width: {min_width:.6g} (same units as CSV 'width') "
...
plt.imshow(
    np.exp(-((δE_f_grid) ** 2) / (2 * sigmaE**2)) / np.sqrt(2 * np.pi * sigmaE**2),
    extent=[δE_f[0] / e, δE_f[-1] / e, δω[0] / (2 * np.pi * 1e12), δω[-1] / (2 * np.pi * 1e12)],
    aspect="auto",
)
plt.colorbar(label="sinc(Δ_PM * L_int / 2π)")
...
dispersion_plot(omega0, v0, v_g_func, recoil_func, E_rel, k_rel)
...
for i, v0_test in enumerate(tqdm(v0_vec, desc="Scanning v_0", position=0)):
    widths_1D_v0[i] = float(
        final_state_probability_density(N, L_int, sigmaE, v0_test, omega0, vg, recoil, gamma_dB_per_cm)[5]
    )
# Save the 1D v0 width data to CSV
df_v0 = pd.DataFrame({"v0": v0_vec, "width": widths_1D_v0})
...
df_sigmaE_loaded = pd.read_csv("SEM\width_vs_L_for_different_sigmaE.csv")

df_loaded = pd.read_csv("SEM\width_vs_L_for_different_losses.csv")
...
for i, label in enumerate(loss_labels):
    ...
    plt.plot(df_plot["L_int"][:-omit_n] / lambda0, df_plot["width"][:-omit_n] / initial_width_loss, color=colors[i])

df_plot = df_loaded[df_loaded["label"] == "0 dB/cm"]
legend_label = f"{FWHM(sigmaE_loss_simulation):.3f} eV"
plt.plot(
    df_plot["L_int"][:-omit_n] / lambda0, df_plot["width"][:-omit_n] / initial_width_loss, color=colors[0], label=" "
)
...
for i, label in enumerate(loss_labels):
    ...
    loss_val = df_plot["loss"].iloc[0]
    legend_label = f"{FWHM(sigmaE_loss_simulation):.3f} eV ({loss_val:.0f} dB/cm)"
    plt.plot(df_plot["L_int"][:-omit_n] / lambda0, df_plot["width"][:-omit_n], color=colors[i])


for sigmaE_val, label, color in zip(sigmaE_values, sigmaE_labels, colors_sigmaE):
    df_plot = df_sigmaE_loaded[df_sigmaE_loaded["label"] == label]
    legend_label = f"{FWHM(sigmaE_val):.3f} eV"
    plt.plot(df_plot["L_int"][:-omit_n] / lambda0, df_plot["width"][:-omit_n], label=" ", color=color)
    L0_loss = 1.18 * 4 * (E0 * v0 / (sigmaE_val * e * omega0))  # optimal interaction length for loss sigmaE
...
fig, axes = plt.subplots(1, 4, sharey=True, figsize=(20, 5), gridspec_kw={"wspace": 0.01})

# Prepare velocity axis in units of c
v0_axis = v0_vec_rounded / c

# Plot for each loss value
for ax, Z_masked, loss in zip(axes, [Z_0db_masked, Z_10db_masked, Z_30db_masked, Z_100db_masked], [0, 10, 30, 100]):
    im = ax.imshow(
        Z_masked,
        origin="lower",
        aspect="auto",
    ...
    ax.axhline(closest_L0 / lambda0, color="tab:green", linestyle="--", label="                ")
    ax.axvline(vg / c, color="tab:red", linestyle="--", label="                               ")
...
for i, sigmaE_val in enumerate(tqdm(sigmaE_values_2d)):
    for j, v0_test in enumerate(tqdm(v0_vec, desc=f"σE={sigmaE_val:.2e}", leave=False)):
        widths_2D_sigmaE_v0[i, j] = float(
            final_state_probability_density(N, L_int, sigmaE_val, v0_test, omega0, vg, recoil, gamma_dB_per_cm)[5]
        )
# Save the 2D sigmaE-v0 width data to CSV
df_sigmaE_v0 = pd.DataFrame(
    widths_2D_sigmaE_v0, index=[f"{val:.6e}" for val in sigmaE_values_2d], columns=[f"{val:.6e}" for val in v0_vec]
)
...
nrm = matplotlib.colors.TwoSlopeNorm(
    vmin=widths_2D_sigmaE_v0_lognorm.min(), vcenter=0, vmax=widths_2D_sigmaE_v0_lognorm.max()
)
