...
for gamma_dB_per_cm, color in zip(loss_labels, colors):
    ...
    widths_2D_all.append(widths_vs_L)

# Save results to CSV
df_loss = pd.DataFrame(results)
df_loss.to_csv("width_vs_L_for_different_losses.csv", index=False)
...
for sigmaE_val, label, color in zip(sigmaE_values, sigmaE_labels, colors_sigmaE):
    widths_vs_L = []
    for L_int_test in tqdm(L_int_vec_log, desc=f"σE={label}", leave=False):
...
v0_factors = np.linspace(0.9, 1.1, 5)  # 5 values around vg
v0_values = v0_factors * vg
v0_labels = [rf"${f:.3f}\,v_g$" if not np.isclose(f, 1.0) else r"$v_g$" for f in v0_factors]
colors_v0 = ["tab:blue", "tab:orange", "tab:green", "tab:red", "tab:purple"]
N = 2**9
results_v0 = []
widths_vs_L_all_v0 = []

for v0_val, label, color in zip(v0_values, v0_labels, colors_v0):
    widths_vs_L = []
    for L_int_test in tqdm(L_int_vec_log, desc=f"v0={label}", leave=False):
        width = float(final_state_probability_density(
            N, L_int_test, sigmaE, v0_val, omega0,
            v_g_func(omega0, v0_val), recoil_func(omega0, v0_val), gamma_dB_per_cm
        )[5])
        widths_vs_L.append(width)
        results_v0.append({
            "v0": v0_val,
    ...
    widths_vs_L_all_v0.append((widths_vs_L, label, color))

df_v0 = pd.DataFrame(results_v0)
df_v0.to_csv("width_vs_L_for_different_v0.csv", index=False)

# Plot directly from simulation results (no CSV load)
plt.figure(figsize=(8, 5))
for (widths_vs_L, label, color), v0_val in zip(widths_vs_L_all_v0, v0_values):
...
plt.yscale("log")
plt.xscale("log")
plt.title(r"Width vs $L_{int}$ for different $v_0$")
plt.legend()
plt.tight_layout()
plt.show()
...
L_int_vec_log = np.logspace(np.log10(1 * L0), np.log10(20 * L0), 20)
omega0_factors = np.logspace(np.log10(0.3), np.log10(2.0), 7)
...
for omega0_val, label, color in zip(omega0_values, omega0_labels, colors_omega0):
    widths_vs_L = []
    for L_int_test in tqdm(L_int_vec_log, desc=f"ω0={label}", leave=False):
        width = float(final_state_probability_density(
...
for omega0_val, label, color in zip(omega0_values, omega0_labels, colors_omega0):
    ...
    # Convert omega0 to THz for legend
    omega0_THz = omega0_val / (2 * np.pi * 1e12)
    legend_label = f"{omega0_THz:.2f} THz" if label != r"$\omega_0$" else r"$\omega_0$"
    plt.plot(df_plot["L_int"], df_plot["width"], marker='.', linestyle='-', label=legend_label, color=color)
plt.axhline(initial_width, color="gray", linestyle=":", label="Initial width")
plt.xlabel("Interaction length $L_{int}$ (m)")
plt.ylabel("Final width (eV)")
plt.yscale("log")
plt.xscale("log")
plt.title(r"Width vs $L_{int}$ for different $\omega_0$ (in THz)")
plt.legend(title=r"$\omega_0$ (THz)")
plt.tight_layout()
plt.show()
# # Save the last figure as SVG
...
loss_values = [0, 0.5, 1, 1.5]
loss_labels = ["0 dB/cm", "5 dB/cm", "10 dB/cm", "1.5 dB/cm"]
colors = ["tab:blue", "tab:orange", "tab:green", "tab:red"]
N = 2**11
# Run simulation and save results to CSV
results = []
...
df_widths_vs_L_loss.to_csv("width_vs_L_for_different_losses_TEM1.csv", index=False)

# Load results from CSV and plot
df_loaded = pd.read_csv("width_vs_L_for_different_losses_TEM1.csv")
...
df_sigmaE.to_csv("width_vs_L_for_different_sigmaE_TEM1.csv", index=False)

# Step 2: Load results from CSV and plot
df_sigmaE_loaded = pd.read_csv("width_vs_L_for_different_sigmaE_TEM1.csv")
plt.figure(figsize=(8, 5))
...
for label, color in reversed(list(zip(loss_labels, colors))):
    ...
    plt.plot(df_plot["L_int"][:-1], df_plot["width"][:-1], marker='.', linestyle='-', label=label, color=color)

for sigmaE_val, label, marker, color in zip(sigmaE_values, sigmaE_labels, markers, colors_sigmaE):
    df_plot = df_sigmaE_loaded[df_sigmaE_loaded["label"] == label]
    legend_label = f"{sigmaE_val:.3f} eV"
    # Exclude the last point
    plt.plot(df_plot["L_int"][:-1], df_plot["width"][:-1], marker=marker, linestyle='-', label=legend_label, markersize=5, color=color)
...
v0_vec = np.unique(
    np.concatenate([
        np.linspace(0.9999, 1.0001, v0_num) * v0,          # ±1%
        np.linspace(0.99999, 1.00001, v0_num // 2) * v0,    # ±0.1%
        [v0],                                           # exact center
    ])
)
...
df_1D_v0 = pd.DataFrame({
    "v0": v0_vec,
    "width": widths_1D_v0
})
df_1D_v0.to_csv("width_vs_v0_TEM.csv", index=False)
...
# Set x-ticks as plain numbers (no scientific notation), 8 digits after the point
num_xticks = 7
xtick_vals = np.linspace(np.min(v0_vec/c), np.max(v0_vec/c), num_xticks)
plt.xticks(xtick_vals, [f"{x:.8f}" for x in xtick_vals])

plt.savefig("width_vs_v0_TEM.svg", format="svg")
plt.show()
