...
# %% 1D GRAPH: width vs L for different losses
L_num = 11
L_int_vec = np.logspace(np.log10(0.001* L0), np.log10(1000* L0), L_num) 
loss_values = [0, 100, 500, 1500]
loss_labels = ["0 dB/cm", "100 dB/cm", "500 dB/cm", "1500 dB/cm"]
...
df_widths_vs_L_loss.to_csv("width_vs_L_for_different_losses_SLOW1.csv", index=False)

# Load results from CSV and plot
df_loaded = pd.read_csv("width_vs_L_for_different_losses_SLOW1.csv")
...
df_sigmaE.to_csv("width_vs_L_for_different_sigmaE_SLOW1.csv", index=False)

# Step 2: Load results from CSV and plot
df_sigmaE_loaded = pd.read_csv("width_vs_L_for_different_sigmaE_SLOW1.csv")
plt.figure(figsize=(8, 5))
...
v0_vec = np.unique(
    np.concatenate([
        np.linspace(0.9, 1.1, v0_num) * v0,          # ±1%
        np.linspace(0.99, 1.01, v0_num // 2) * v0,    # ±0.1%
        [v0],                                           # exact center
    ])
)
