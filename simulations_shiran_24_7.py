...
#%% 
def final_state_probability_density(initial_width,L_int, gamma, recoil, v_g, v_0, omega_0):

    Delta_PM = k(E_f + δE_f_grid + hbar * δω_grid) - k(E_f + δE_f_grid - hbar * omega0) - (q0 + (δω_grid / v_g) + 0.5 * recoil * δω_grid**2)
...
res = [[k_rel(E0 + hbar * δω_) - k_rel(E0-hbar*omega0) - (q0 + (δω_ / v_g) + 0.5 * recoil * δω_**2) for recoil in recoil_vec] for δω_ in δω]
plt.figure()
plt.imshow(res, extent=[δω.min(), δω.max(), recoil_vec.min()/e, recoil_vec.max()/e],
...
for L_int_test in L_int_vec:
    rho_f = np.sum((e**2 * hbar * L_int_test**2 / (2*(δω_grid+omega0)*m**2))*(1/np.sqrt(2*np.pi*deltaE**2))*np.exp(-(δE_f_grid + hbar*δω_grid)**2/2/deltaE**2)*(np.sinc(Delta_PM*(L_int_test/2)/np.pi))**2,axis = 1)*dω
