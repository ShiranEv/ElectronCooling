...
def final_state_probability_density(N,
    ...
    δω = np.linspace(- 4*sigmaE , 4*sigmaE, N)/hbar
    dω = δω[1] - δω[0]
    dE = dω * hbar
    ...
    # print( e**2 *hbar * v_g**2 * L_int**2 )
    # factor = e**2 *hbar * v_g**2 * L_int**2  / (2*eps0*(δω_grid + omega0))
    ...
    # p1 = np.sum(rho_f * dE)
    # print(f"p1={p1}")
    # rho_f_total = rho_f + (1 - p1)*rho_i_1d
    ...
    rho_f = np.sum((1/np.sqrt(2*np.pi*sigmaE**2))*np.exp(-(δE_f_grid + hbar*δω_grid)**2/2/sigmaE**2)*(np.sinc(Delta_PM*L_int/2/np.pi))**2,axis = 1)*dω
    rho_f = rho_f / np.sum(rho_f * dE)  # Normalize the final state probability density

    rho_f_p = np.sum((1/np.sqrt(2*np.pi*sigmaE**2))*np.exp(-(δE_f_grid + hbar*δω_grid)**2/2/sigmaE**2)*(np.sinc(Delta_PM*L_int/2/np.pi))**2,axis = 0)*dE
    rho_f_p = rho_f_p / np.sum(rho_f_p * dω)  # Normalize the final state probability density
    final_width_eV = compute_FWHM(δE_f, rho_f)/e
    print(f"final_width_eV = {final_width_eV}") 
    rho_i = np.exp(-δE_f**2/2/sigmaE**2)/ np.sqrt(2*np.pi*sigmaE**2)  # Initial state probability density
    return δE_f, rho_f, final_width_eV, rho_i, δω, rho_f_p
...
def simulation_test(N, v0, sigma_eV , Lambda, L_interaction, k_function , E_function,v_function, Grid_factor):
    ...
    δE_f_eV, rho_f_e, final_width_FWHM, rho_i_e, δω, rho_f_p = final_state_probability_density(
        ...
        L_interaction,
        v0,
        recoil_func(omega0, v0, E_function, k_function),
        v0,
        omega0,
        Grid_factor
    )
...
initial_width = Delta_E_initial * 2 * np.sqrt(2 * np.log(2)) 
print(f"Initial width (FWHM): {initial_width:.4f} eV")
omega0 = 2 * np.pi * c / lambda0
E0 = E_rel(v0)  # central electron energy (J)

simulation_test(N, v0, Delta_E_initial, lambda0 , L_int, k_rel ,E_rel, v_func ,4)
...
for L_int_test in tqdm(L_int_vec, desc="Scanning L_int"):
    width = final_state_probability_density(
        ...
        Delta_E_initial,
        L_int_test,
        v_g_func(omega0,v0,E_func,k_func),
